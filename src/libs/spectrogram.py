...
class pcen(nn.Module):
    def __init__(self, sr=22050, hop_length=512, gain=0.98, bias=2, power=0.5,time_constant=0.400, eps=1e-6, b=None, ref=None):
        ...
        self.filter_a = torch.nn.Parameter(torch.tensor([1, self.b - 1], dtype=torch.float, device=cuda0), requires_grad=False)
        self.filter_b = torch.nn.Parameter(torch.tensor([self.b, 0], dtype=torch.float, device=cuda0), requires_grad=False)
